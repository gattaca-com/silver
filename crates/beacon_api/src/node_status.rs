...
impl NodeStatus {
    /// Fork choice seeds the anchor as executed, so the head it reports is not
    /// optimistic; no target exists until the control tile picks one.
    pub fn at_anchor(head_slot: u64) -> Self {
        Self {
