...
use silver_common::{
    SubReservationError, SubReservationRef, TCacheProducer, TCacheRead, TProducer, TReservation,
...
impl CellStore {
    ...
    pub fn reserve_full(&mut self, len: usize) -> Result<TReservation, StoreError> {
        self.producer.reserve(len, false).ok_or(StoreError::CacheFull)
    }
    ...
    pub fn admit_context(
        ...
        let ssz = {
            let Some(mut reservation) = self.producer.reserve(data.encoded_len(), false) else {
                DataColumnCounters::CellStoreCacheFull.inc();
        ...
        for (position, &column) in self.config.column_indices.iter().enumerate() {
            match data.reserve_column(context, column, &mut self.producer) {
                Ok(reference) => {
                    let view = self.producer.view_sub_reservation(reference).expect("new assembly");
                    assembly_bytes += view.len();
                    let complete = context.blob_count == 0;
                    if complete {
                        view.finish().expect("empty column");
                    }
        ...
        self.counts.bytes += assembly_bytes;
    ...
    pub fn advance(&mut self, now: Instant, min_slot: u64, mut on_expired: impl FnMut(CellKey)) {
        ...
        if slot_changed {
            debug_assert_eq!(self.counts.contexts, 0);
            self.counts.cells = 0;
            self.counts.bytes = 0;
            self.counts.full_bytes = 0;
            self.counts.active_slots = 0;
        }
        if self.dirty {
            self.publish_gauges();
            self.dirty = false;
