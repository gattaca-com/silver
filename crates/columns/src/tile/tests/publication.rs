use silver_common::{
    ...
    ssz_view::{
        BEACON_BLOCK_BODY_FIXED, DATA_COLUMN_SIDECAR_GLOAS_MIN, EXECUTION_PAYLOAD_BID_MIN,
        SidecarLayout,
    },
};
...
fn column_publications_name_the_sidecar_for_gossip_and_following_rpc() {
    ...
    for (source, following, index) in [
        ...
        let out = rig.drain();
        if following {
            let column = SidecarIdentity {
                slot: SLOT,
                block_root,
                column_index: index,
                layout: SidecarLayout::Gloas,
            };
...
fn fulu_column_publication_requires_a_resolved_proposer() {
    ...
    for (slot, relay_eligible) in [(7, true), (2 * SLOTS_PER_EPOCH + 1, false)] {
        ...
        if relay_eligible {
            let column =
                SidecarIdentity { slot, block_root, column_index: 3, layout: SidecarLayout::Fulu };
...
fn held_columns_do_not_request_publication_again() {
    ...
    rig.turn();
    let column =
        SidecarIdentity { slot: SLOT, block_root, column_index: 3, layout: SidecarLayout::Gloas };
