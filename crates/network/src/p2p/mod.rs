...
pub struct P2p {
    ...
    // Peer queues and Quinn owners must drop before this stable pool.
    segmented_frames: Option<Box<SegmentedFramePool>>,
...
impl P2p {
        ...
        E: FnMut(NetEvent),
    {
        ...
        if let Some(pool) = &self.segmented_frames {
            pool.publish_gauges();
        }
        did_work
    }
    ...
    pub fn enqueue_segmented_gossip(
        ...
        match self.peers.get_mut(&ConnectionHandle(peer_id)) {
            Some(peer) => peer.send_segmented_gossip(
                frame,
                context,
                self.segmented_frames.get_or_insert_with(Box::default),
                &mut self.rpc_codec_pool,
            ),
            None => SendResult::UnknownPeer,
        }
