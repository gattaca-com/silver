            ...
            |x| {
                x.iter_batched(
                    || {
                        ...
                        let go_producer = TCache::producer(TCacheId::ControlGossip, 32);
                        ...
                        let tcaches = TCacheTable::from_iter(
                            [&go_producer, &rpc_out, &control_rpc, &cluster_out]
                                .map(|p| p.cache_ref()),
                        );
                        ...
                        for n in 0..i {
                            ...
                            let gi_producer = TCache::producer(TCacheId::NetworkIngress, 32);
                            let mut go_producer =
                                TCache::producer(TCacheId::ControlGossip, 2 << 28);
                            let rpc_in = TCache::producer(TCacheId::NetworkProcessing, 32);
                            let rpc_out = TCache::producer(TCacheId::StorageDelivery, 32);
                            let control_rpc = TCache::producer(TCacheId::ControlRpc, 32);
                            // dummys
