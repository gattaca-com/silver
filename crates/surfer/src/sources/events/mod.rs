...
//! folds the events into per-block traces.
...
pub use stf::StfSpan;
#[cfg(test)]
pub(crate) use trace::tests as trace_tests;
...
impl BlockTraces {
    fn fold(&mut self, clock: &SlotClock, event: StageEvent) {
        let trace = match self.0.iter_mut().rev().find(|r| r.block_root == event.block_root) {
            Some(trace) => trace,
    ...
    /// Largest strip end offset, so an axis covers every visible bar.
    pub fn max_strip_offset(&self, clock: &SlotClock) -> Nanos {
        self.0
            .iter()
            .filter_map(|r| r.offset_in_slot(clock, r.interval(Span::Strip)?.end))
            .max()
            .unwrap_or(Nanos(0))
    }
}
...
impl Events {
    pub fn open(base_dir: &Path, genesis_unix_secs: u64, slot_ms: u64) -> Self {
        let tile = EventsTile {
            ...
            traces: BlockTraces(VecDeque::with_capacity(ROWS_CAP)),
        };
