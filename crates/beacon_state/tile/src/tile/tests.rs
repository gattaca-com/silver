...
fn the_published_state_names_the_block_it_was_applied_from() {
    ...
    // The slot tier as the STF leaves it: the `process_slot` out of slot 10
    // recorded the anchor block's root, then `process_block_header` installed
    // the child's header with the zero `state_root` the block arrived with.
    let child_slot_idx = {
        let mut g = tile.state.write();
        let mut sw = g.slot_states.roll_from(tile.last_applied.slot_idx);
        sw.push_block_root(ANCHOR_ROOT);
        sw.advance_slot();
    ...
    tile.publish_applied_block(
        ...
        &[],
        AppliedBlock {
            id: StateId { slot_idx: child_slot_idx, ..tile.last_applied },
            justified: anchor_cp,
            finalized: anchor_cp,
    ...
    let (state_root, recorded) = tile
        ...
        .read(&|v| {
            (
                ...
                v.slot.block_root_proposed_at(CHILD_SLOT),
            )
        })
        .unwrap();
