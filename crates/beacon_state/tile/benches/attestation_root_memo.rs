...
//! head splits; D=5000 models adversarial distinct-value spray past the
//! memo's cap — the memoized cost must stay within lookup-overhead of the
//! unmemoized baseline there).
//!
...
fn bench_root_derivation(c: &mut Criterion) {
    ...
    for distinct in [2usize, 64, 5000] {
