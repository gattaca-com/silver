...
/// `BlobParameters` (Fulu BLOB_SCHEDULE entry) lives in `silver_common`
/// because it's per-network spec config, not state-shape. Re-export so
/// existing `beacon_state::types::BlobParameters` references keep working.
pub use silver_common::BlobParameters;
