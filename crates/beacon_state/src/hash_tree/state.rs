...
impl HashTreeState {
    ...
    pub fn set_leaf(&mut self, i: usize, leaf: B256) {
        ...
        debug_assert!(i < max_elements, "leaf index out of range: i={i}, max_elements={}", max_elements);
        self.delta = self.finalized().set_delta_leaf_in_range(&self.delta, i as u32, 0, max_elements as u32, leaf);
    }
    ...
    /// `prune_to_base` on every fork (including this one) so unused delta nodes are freed.
