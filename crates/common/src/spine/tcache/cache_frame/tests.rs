...
fn frame_reader(gossip: &Producer, columns: Option<&Producer>) -> TCacheReader {
    ...
    reader.open(TCacheId::OutgoingGossip, "", TReadMode::Strict).unwrap();
    if columns.is_some() {
        reader.open(TCacheId::DataColumns, "", TReadMode::Retained).unwrap();
    }
    reader
}
...
fn copy_handle_round_trips_framing_and_source_ranges() {
    ...
    let mut producer = TCache::producer(TCacheId::OutgoingGossip, 1 << 18);
...
fn shared_segments_expose_only_verified_subranges() {
    let mut producer = TCache::producer(TCacheId::OutgoingGossip, 1 << 18);
    let mut columns = TCache::producer(TCacheId::DataColumns, 1 << 18);
    ...
    reader.advance_retention(TCacheId::DataColumns, columns.next_seq());
    assert!(view.segments().next().unwrap().acquire(&mut reader).is_none());
    assert_eq!(ranges[0].as_ref(), b"el");
}
...
fn descriptor_bounds_and_sources_are_checked() {
    let mut producer = TCache::producer(TCacheId::OutgoingGossip, 1 << 18);
    let mut other = TCache::producer(TCacheId::SszGossip, 1 << 18);
    let mut reader =
    ...
    reader.open(TCacheId::SszGossip, "", TReadMode::Retained).unwrap();
...
fn failed_acquisition_releases_every_successful_prefix_without_touching_other_reads() {
    ...
    for failed in 0..segments.len() {
        ...
        assert_eq!(reader.active_count(TCacheId::OutgoingGossip), 1, "failed descriptor {failed}");
        assert_eq!(reader.active_count(TCacheId::DataColumns), 1, "failed descriptor {failed}");
        assert_eq!(gossip_guard.buffer().unwrap().0, b"gossip");
        assert_eq!(column_guard.buffer().unwrap().0, b"column");
    }
    drop(gossip_guard);
    drop(column_guard);
    assert_eq!(reader.active_count(TCacheId::OutgoingGossip), 0);
    assert_eq!(reader.active_count(TCacheId::DataColumns), 0);
}
...
fn handoff_transfers_counts_and_frame_drop_releases_only_the_remainder() {
    ...
    assert_eq!(reader.active_count(TCacheId::OutgoingGossip), 2);
    assert_eq!(reader.active_count(TCacheId::DataColumns), 2);
    ...
    reader.advance_retention(TCacheId::DataColumns, columns.next_seq());
    drop(frame);
    assert_eq!(reader.active_count(TCacheId::OutgoingGossip), 1);
    assert_eq!(reader.active_count(TCacheId::DataColumns), 1);
...
fn shared_handoff_survives_closure_without_exposing_unverified_gaps() {
    ...
    let mut frame =
        reference.acquire(&mut reader, now).unwrap().acquire_segments(&mut reader).unwrap();
    assert_eq!(reader.active_count(TCacheId::DataColumns), 4);
    ...
    for (index, expected) in [&[0; 3][..], &[2; 4], &[10; 2], &[12; 1]].into_iter().enumerate() {
        ...
        assert_eq!(reader.active_count(TCacheId::DataColumns), 4 - index);
        assert_eq!(range.as_ref(), expected);
        drop(range);
        assert_eq!(reader.active_count(TCacheId::DataColumns), 3 - index);
    }
...
fn coalescing_transfers_one_pin_and_releases_redundant_pins() {
    ...
    let mut frame =
        reference.acquire(&mut reader, now).unwrap().acquire_segments(&mut reader).unwrap();
    assert_eq!(reader.active_count(TCacheId::OutgoingGossip), 4);
    let Some(AcquiredCacheSegment::Data(range)) = frame.take_next() else { panic!() };
    assert_eq!(reader.active_count(TCacheId::OutgoingGossip), 2);
    assert!(frame.take_next().is_none());
    drop(frame);
