...
mod acquired;
pub use acquired::{AcquiredGossipFrame, AcquiredGossipSegment};
...
impl GossipFrameRef {
    pub fn write(
        ...
        let mut reservation =
            producer.reserve(descriptor_len, false).ok_or(GossipFrameError::CacheFull)?;
...
impl GossipFrameView {
    ...
    ) -> Option<AcquiredGossipFrame> {
        AcquiredGossipFrame::new(self, gossip, columns)
    }
    ...
    fn segment(&self, index: usize) -> GossipFrameSegment {
        ...
        GossipFrameSegment::decode(&buffer[start..start + SEGMENT_BYTES], self.framing_start)
    }
}
