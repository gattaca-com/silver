...
fn update_head_assigns_the_head_but_advances_finalization_only_forward() {
    ...
    assert_eq!((store.head.slot, store.head.root), (64, [0xAA; 32]));
    assert_eq!((store.head.finalized_slot, store.head.finalized_root), (32, [0x11; 32]));

    store.update_head(65, [0xBB; 32], 32, [0x11; 32]);
    assert_eq!((store.head.slot, store.head.root), (65, [0xBB; 32]), "the head still moves");
    assert_eq!(
        (store.head.finalized_slot, store.head.finalized_root),
        (32, [0x11; 32]),
        "a repeated finalization changes nothing"
    );

    store.update_head(96, [0xCC; 32], 64, [0x33; 32]);
    assert_eq!((store.head.finalized_slot, store.head.finalized_root), (64, [0x33; 32]));
