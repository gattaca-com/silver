...
# Run surfer (the metrics TUI) with `#[timed]` perf counters folded into the
# flamegraph. Counter labels come from the watched silver's published schema
# (it writes the event names next to its rings), so no event list is needed
# here. Extra args pass through as `[BASE_DIR] [APP_NAME]`.
surfer *args='':
  cargo run --release -p silver_surfer --features perf -- {{args}}
